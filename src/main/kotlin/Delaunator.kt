...
class Delaunator(private val points: List<PointInt>) {

    private val epsilon = 2.0.pow(-52.0)
    private val edgeStack = Array(512) { 0 }

    private var triangles: Array<Int>
    private var halfedges: Array<Int>
    ...
    init {
        ...
        points.forEachIndexed { index, point ->
            coords[2 * index] = point.x.toDouble()
            coords[2 * index + 1] = point.y.toDouble()
        }
        ...
        val maxX = Double.POSITIVE_INFINITY
        val maxY = Double.POSITIVE_INFINITY
        ...
        this.cx = center.x.toDouble()
        this.cy = center.y.toDouble()

        val dists = Array(n) { i ->
            dist(coords[2 * i], coords[2 * i + 1], center.x.toDouble(), center.y.toDouble())
        }
    ...
    private fun createHull(points: List<PointInt>): List<Edge> {
        return points.mapIndexed { index: Int, point: PointInt ->
    ...
    private fun getHullPoints(): List<PointInt> {
    ...
    private fun getTriangleCenter(t: Int): PointInt {
        val vertices = getTrianglePoints(t)
        return getCentroid(vertices)
    }

    private fun getCentroid(points: List<PointInt>): PointInt {
        ...
        accumulatedArea *= 3.0
        return PointInt(
            (centerX / accumulatedArea).toInt(),
            (centerY / accumulatedArea).toInt(),
        )
    }

    private fun getTrianglePoints(t: Int): List<PointInt> {
...
data class Edge(val index: Int, val p1: PointInt, val p2: PointInt)

data class VoronoiCell(val index: Int, val points: List<PointInt>)
